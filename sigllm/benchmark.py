    ...
    if isinstance(hyperparameters, dict):
        ...
        if pipeline_path:
            ...
            file_path = os.path.join(
                PIPELINE_DIR, 'prompter', f'prompter_{dataset_name.lower()}.json'
            )
            print(file_path)
            if os.path.exists(file_path):
                with open(file_path) as f:
                    hyperparameters_ = json.load(f)
                print("found hyperparams:", hyperparameters_)
...
def _evaluate_signal(
    ...
    print('now running sigllm on signal', signal)
    print(f'taking multivariate version multivariate/{signal}')
    
    _, test = _load_signal('multivariate/' + signal, test_split)
    # test = test[:145]            ####################################################### IF YOU WANT TO CHANGE FOR TEST PURPOSES (SMALL DATA SIZE) IT IS HERE
    ...
    for dataset, signals in datasets.items():
        ...
        for pipeline_name, pipeline in pipelines.items():
            ...
            for signal in signals:
                if working_signals is not None and signal not in working_signals:
                    print(f'skipping signal {signal}')
                    continue
...
def main(pipelines, datasets, resume, workers, output_path, cache_dir, anomaly_dir, working_signals, **kwargs):
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--working_signals', nargs='+', type=str, default=None)
